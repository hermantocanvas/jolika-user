...
const BuyerOrders = () => {
  ...
  async function loadOrdersOfBuyer(pageNumber) {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_APIURL}api/v1/orders/buyer/get/${chosenOrderId}/${chosenStatus}/${pageNumber}`
      );
      ...
      return (
        <li className={classList} key={number}>
          <Link
            to="#"
            className={classLink}
            onClick={() => loadOrdersOfBuyer(number)}
          >
            {number}
          </Link>
        </li>
      ...
      <div className="details-wrap">
        <div className="details-box orders">
          ...
          <table className="table table-striped noMoreTable">
            ...
            <tbody>
              {orders.map((order) => (
                <OrderItem key={order._id} order={order} userType="buyer" />
              ))}
            </tbody>
          </table>
          <div className="pagination-wrapper" style={{ borderTop: "none" }}>
            <ul className="pagination">
              <li>
                <Link to="#" onClick={() => loadOrdersOfBuyer(1)}>
                  <i className="fa fa-angle-double-left"></i>
                </Link>
              </li>
              {renderPageNumbers}
              <li>
                <Link to="#" onClick={() => loadOrdersOfBuyer(total_pages)}>
